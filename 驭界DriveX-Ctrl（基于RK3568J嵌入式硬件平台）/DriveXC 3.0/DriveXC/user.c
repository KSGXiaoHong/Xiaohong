...
static void* RecvToServer_thread(void* arg)
{
    ...
    while(!exit_client_fg)//判断是否退出
    {
        ...
        if(FD_ISSET(socket_fd, &readfds))
        {
            ...
            //从服务器读
            ssize_t recv_len = recv(socket_fd, buf_recv, sizeof(buf_recv)-1, 0);
            if (recv_len <= 0) 
            {
                perror("接收失败或服务器断开");
                exit_client_fg = 1;//同步退出
                break;//退出线程
            }
            buf_recv[recv_len] = '\0';  // 确保正确终止
...
static void* SendToServer_thread(void* arg)
{
    ...
    {//循环检测写标志位是否就绪
        if(ready_send)//写数据标志位就绪
        {
            ...
            {
                creat_flag = 1;//创建线程标志位置1
                ready_read = 1;//读数据就绪 标志位置1
                exit_client_fg = 1;//线程同步退出
