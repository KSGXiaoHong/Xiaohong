...
void def()
{
    ...
    lv_obj_set_scrollbar_mode(win, LV_SCROLLBAR_MODE_OFF);// 隐藏滚动条
    lv_obj_set_scroll_dir(win,LV_DIR_HOR);//仅允许水平滑动
    //创建功能按键7个
    lv_obj_t* ctl_button[CTRL_NUM];
    //给按键添加图标
    lv_obj_t* app;
    ...
    lv_obj_add_event_cb(ctl_button[6], kongtiao, LV_EVENT_CLICKED,NULL);//空调
    /*视频回调：end*/
    /**********功能按键：end**********/
    
    /**********上锁按钮：begin**********/
    lv_obj_t* lock_button = lv_button_create(p);
    lv_obj_t* lock_pt = lv_image_create(lock_button);//锁的图片
    lv_obj_add_style(lock_button,&style_ctl_button, LV_STATE_DEFAULT);//应用车内控制按键样式
    lv_obj_align(lock_button,LV_ALIGN_TOP_LEFT,900,460);
    lv_obj_center(lock_pt);
    lv_obj_add_event_cb(lock_button,car_lock,LV_EVENT_CLICKED,NULL);//注册事件
    if(lock_flag == 0)//上锁的路径
        lv_image_set_src(lock_pt,"A:/XiaoHong/photo/car_lock.bmp");//显示锁的图片
    if(lock_flag == 1)//解锁的路径
        lv_image_set_src(lock_pt,"A:/XiaoHong/photo/car_unlock.bmp");//显示锁的图片
    /**********上锁按钮：end**********/

    /**********空调按键：begin**********/
    lv_obj_t* kt_button = lv_button_create(p);//空调按键
    lv_obj_t* kt_pt = lv_image_create(kt_button);//空调的图片
    lv_image_set_src(kt_pt,"A:/XiaoHong/photo/kongtiao.bmp");//显示图片
    lv_obj_add_style(kt_button,&style_ctl_button, LV_STATE_DEFAULT);//应用车内控制按键样式
    lv_obj_align(kt_button,LV_ALIGN_TOP_LEFT,668,460);//位置
    lv_obj_center(kt_pt);//图片居中
    lv_obj_add_event_cb(kt_button, kongtiao, LV_EVENT_CLICKED,NULL);//绑定空调函数
    /**********空调按键：begin**********/
...
void menu()
{
    ...
    //注册事件,绑定到按键，松手执行回调函数
    lv_obj_add_event_cb(button[0], help, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[1], Canshu, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[2], video, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[3], photos, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[4], everyday, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[5], play, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[6], map, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[7], user, LV_EVENT_CLICKED,NULL);
    lv_obj_add_event_cb(button[8], other, LV_EVENT_CLICKED,NULL);
}
